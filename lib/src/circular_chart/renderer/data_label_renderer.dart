...
class _CircularDataLabelPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_CircularDataLabelPainter oldDelegate) {
    return true;
  }
}
...
void _renderDataLabel(CircularSeries<dynamic, dynamic> series, Canvas canvas,
  ...
  // series._clipAndTranslate(canvas, chart, series);
...
void _drawLabel(
  ...
  if (dataLabel.builder == null) {
    ...
    if (strokeWidth != null && strokeWidth > 0) {
      rectPaint = Paint()
        ..color = strokeColor.withOpacity(animateOpacity)
        ..style = PaintingStyle.stroke
    ...
    textStyle.color = (dataLabel.textStyle.color ??
            _getSaturationColor(dataLabel.useSeriesColor
                ? point.fill
                : (dataLabel.color ?? series is RadialBarSeries
                    ? Colors.white
                    : point.renderPosition == LabelPosition.inside
                        ? point.fill
                        : themeColor)))
