...
class CircularSeries<T, D> extends ChartSeries<T, D> {
  ...
  // void _clipAndTranslate(Canvas canvas, SfCircularChart chart,
  //     CircularSeries<dynamic, dynamic> series) {
  //   if (chart._needToMoveFromCenter &&
  //       series.pointRadiusMapper == null &&
  //       chart.centerX == '50%' &&
  //       chart.centerY == '50%') {
  //     final Rect areaRect = chart._chartState.chartAreaRect;
  //     final Rect circularRect = series._rect;
  //     final Size circularSize = Size(circularRect.width, circularRect.height);
  //     canvas.translate(
  //         (-circularRect.left + areaRect.left) +
  //             (areaRect.width / 2) -
  //             (circularSize.width / 2),
  //         (-circularRect.top + areaRect.top) +
  //             (areaRect.height / 2) -
  //             (circularSize.height / 2));
  //   } else {
      ...
      if (chart._chartState.selectionData.isNotEmpty) {
        if (chart.onSelectionChanged != null) {
          chart.onSelectionChanged(
              getSelectionEventArgs(series, seriesIndex, currentPointIndex));
        }
    ...
    if (!isDynamicUpdate ||
      ...
      if (cornerStyle != CornerStyle.bothFlat) {
        ...
        if (cornerStyle == CornerStyle.startCurve ||
          ...
          actualStartAngle = pointStartAngle;
        }
