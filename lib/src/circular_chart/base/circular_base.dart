...
class _SfCircularChartState extends State<SfCircularChart>
  ...
  List<Widget> _chartWidgets; // List of container widgets for chart series
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      child: GestureDetector(
        ...
        child: Column(
          children: <Widget>[_renderChartTitle(), _renderChartElements()],
        ),
      )),
    );
  }
  ...
  Widget _renderChartElements() {
    return Expanded(
      child: LayoutBuilder(
        ...
        final List<Widget> legendTemplates = _bindLegendTemplateWidgets();
  ...
  List<Widget> _bindLegendTemplateWidgets() {
    Widget legendWidget;
    ...
    for (int i = 0; i < chart._chartSeries.visibleSeries.length; i++) {
      ...
      if (series.selectionSettings != null) {
        if (chart.initialSelectedDataIndexes.isNotEmpty) {
          for (int index = 0;
              index < chart.initialSelectedDataIndexes.length;
              index++) {
            chart._chartState.selectionData
                .add(chart.initialSelectedDataIndexes[index].pointIndex);
          }
        }
      }
