...
class _PyramidDataLabelPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_PyramidDataLabelPainter oldDelegate) {
    return true;
  }
}
    ...
    if (!_isPyramidLabelItersect(rect, lastRenderedLabelRegion) &&
      ...
      if (pointIndex != 0) {
        ...
        final Rect previousRenderedRect =
            renderDatalabelRegions[renderDatalabelRegions.length - 1];
        rect = Rect.fromLTWH(
            rect.left, previousRenderedRect.bottom, rect.width, rect.height);
        labelLocation = Offset(
...
bool _isPyramidLabelItersect(Rect rect, Rect previousRect) {
  ...
  if (previousRect != null && rect.top < previousRect.bottom) {
    isIntersect = true;
  }
  return isIntersect;
}
...
void _drawPyramidLabel(
  ...
  if (dataLabel.builder == null) {
    ...
    renderDataLabelRegions.add(Rect.fromLTWH(
        location.dx, location.dy, labelRect.width, labelRect.height));
  }
}
