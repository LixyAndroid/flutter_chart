...
Color _getDataLabelSaturationColor(_CartesianChartPoint<dynamic> currentPoint,
  ...
  final CartesianLabelPosition labelPosition =
      (series._seriesType == 'rangecolumn' &&
              (dataLabel.position == CartesianLabelPosition.bottom ||
                  dataLabel.position == CartesianLabelPosition.middle))
          ? CartesianLabelPosition.auto
          : dataLabel.position;
...
void _animateStackedRectSeries(
  ...
  if (seriesIndex != 0) {
    for (int j = 0; j < chart._chartSeries.clusterStackedItemInfo.length; j++) {
