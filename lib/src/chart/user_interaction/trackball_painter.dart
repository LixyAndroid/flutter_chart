...
class _TrackballPainter extends CustomPainter {
  ...
  Paint getLinePainter(Paint trackballLinePaint) {
    return trackballLinePaint;
  }
    ...
    for (int index = 0; index < chartPointInfo.length; index++) {
      ...
      if (chart.trackballBehavior.tooltipDisplayMode ==
        ...
        for (int i = 0; i < chartPointInfo.length; i++) {
          if (chart.onTrackballPositionChanging != null) {
            chartPoint = TrackballArgs();
            chartPoint.chartPointInfo = chartPointInfo[i];
            chart.onTrackballPositionChanging(chartPoint);
            chartPointInfo[i].label = chartPoint.chartPointInfo.label;
          }
      ...
      if (x != null &&
          ...
          if (isHorizontalOrientation) {
            ...
            if (xPos < boundaryRect.left) {
              if (chart.trackballBehavior.tooltipDisplayMode ==
                  ...
                  chart.trackballBehavior.tooltipAlignment ==
                      ChartAlignment.far) {
                xPos = boundaryRect.left + groupAllPadding;
              } else {
                xPos = boundaryRect.left;
              }
              ...
              if (chart.trackballBehavior.tooltipDisplayMode ==
                  ...
                  chart.trackballBehavior.tooltipAlignment ==
                      ChartAlignment.far) {
                xPos = xPos - labelRect.width / 2 - groupAllPadding;
              } else {
                xPos = (xPos - labelRect.width - (2 * padding)) -
                    2 * pointerLength;
              }
              isRight = true;
            } else {
        ...
        drawBackground(canvas, labelRect, nosePointX, nosePointY, borderRadius,
  ...
  void drawTrackBallLine(Canvas canvas, Paint paint, int index) {
    ...
    if (chart.trackballBehavior.lineType == TrackballLineType.vertical) {
      ...
      //       ? drawDashedLine(paint, canvas, dashArrayPath)
  ...
  void drawDashedLine(Paint paint, Canvas canvas, Path dashArrayPath) {
    bool even = false;
    for (int i = 1;
        ...
        if (series._visible) {
          ...
          if (!seriesAxes.contains(chartAxis)) {
            ...
            for (XyDataSeries<dynamic, dynamic> axisSeries
              ...
              validateNearestXValue(leastX, series, position.dx, position.dy);
            }
            if (chartPointInfo.isNotEmpty) {
              if (chart.trackballBehavior.tooltipDisplayMode !=
                  TrackballDisplayMode.groupAllPoints) {
                if (chart._requireInvertedAxis) {
                  chartPointInfo.sort((_ChartPointInfo a, _ChartPointInfo b) =>
              ...
              if (chart.trackballBehavior.tooltipDisplayMode ==
                  ...
                  (series._isRectSeries &&
                      chart.trackballBehavior.tooltipDisplayMode !=
                          TrackballDisplayMode.groupAllPoints)) {
                validateNearestPointForAllSeries(
  ...
  void validateNearestPointForAllSeries(double leastX,
    ...
    for (int i = 0; i < tempTrackballInfo.length; i++) {
      ...
      if (pointInfo.series._isRectSeries &&
        ...
        if (isXAxisInverse) {
          ...
          isStartIndex = isTemp;
        } else if (xPos < leastX && isStartIndex) {
  ...
  void addChartPointInfo(CartesianSeries<dynamic, dynamic> series, double xPos,
  ...
  String getFormattedValue(num value) {
