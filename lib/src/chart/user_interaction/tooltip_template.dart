  ...
  Widget build(BuildContext context) {
    ...
    if (widget.show) {
      if (needMeasure) {
        ...
        SchedulerBinding.instance.addPostFrameCallback((_) => loaded());
      } else {
        ...
        final Offset location =
            getTooltipLocation(tooltipRect, widget.clipRect);
    ...
    if (widget.show) {
      tooltipTimer = Timer(const Duration(milliseconds: 2000), tooltipClose);
    }
    return tooltipWidget;
  }

  Offset getTooltipLocation(Rect tooltipRect, Rect bounds) {
