...
class SfCartesianChart extends StatefulWidget {
  ...
  ///        child: SfCartesianChart(
  ///            zoomPanBehavior: ZoomPanBehavior(enableSelectionZooming: true),
  ///            onZoomEnd: (ZoomingArgs args) => zoom(args),
  ///        ));
  ///}
  ///dynamic zoom(ZoomingArgs args) {
  ...
  ///        child: SfCartesianChart(
  ///            zoomPanBehavior: ZoomPanBehavior(enableSelectionZooming: true),
  ///            onZoomReset: (ZoomingArgs args) => zoom(args),
  ///        ));
  ///}
  ///dynamic zoom(ZoomingArgs args) {
  ///    print(args.currentZoomPosition);
  ...
  ///        child: SfCartesianChart(
  ///            zoomPanBehavior: ZoomPanBehavior(enableSelectionZooming: true),
  ///            onZooming: (ZoomingArgs args) => zoom(args),
  ///        ));
  ///}
  ///dynamic zoom(ZoomingArgs args) {
  ///    print(args.currentZoomPosition);
...
class _SfCartesianChartState extends State<SfCartesianChart>
  ...
  ValueNotifier<int> zoomRepaintNotifier; // Repaint notifier for zoom container
  ValueNotifier<int> axisRepaintNotifier; // Repaint notifier for axis container
  ValueNotifier<int>
      seriesRepaintNotifier; // Repaint notifier for series container
  ValueNotifier<int>
      tooltipRepaintNotifier; // Repaint notifier for tooltip container
  ValueNotifier<int>
      trackballRepaintNotifier; // Repaint notifier for trackball container
  ValueNotifier<int>
      crosshairRepaintNotifier; // Repaint notifier for crosshair container
  ...
  Rect containerRect; // Contains chart container size
  bool zoomProgress;
  ...
  Widget build(BuildContext context) {
    ...
    return _ChartContainer(
      ...
      child: Container(
          ...
          child: Column(
            children: <Widget>[renderChartTitle(), _renderChartElements()],
          )),
    ));
  }

  ///
  /// To render the chart title
  Widget renderChartTitle() {
    Widget titleWidget;
  ...
  Widget _renderChartElements() {
    return Expanded(
      child: LayoutBuilder(
        ...
        final List<Widget> legendTemplates = bindLegendTemplateWidgets();
  ...
  List<Widget> bindLegendTemplateWidgets() {
    Widget legendWidget;
  ...
  void initialize(BoxConstraints constraints) {
    ...
    for (int i = 0; i < widget.series.length; i++) {
      ...
      if (widget._chartState.initialRender) {
        ...
        widget.series[i]._visible =
            checkWithLegendToggleState(i, widget.series[i]._visible);
      }
      widget._chartSeries.visibleSeries.add(widget.series[i]);
    }
  }

  bool checkWithLegendToggleState(int seriesIndex, bool seriesVisible) {
    bool seriesRender;
...
class _ContainerArea extends StatelessWidget {
  ...
  void _renderTemplates() {
    ...
    for (int i = 0; i < chart.series.length; i++) {
      final CartesianSeries<dynamic, dynamic> series = chart.series[i];
      if (!series._visible) {
        for (int index = 0;
      ...
      if (_series != null && _series._visible) {
        ...
        if (_series.selectionSettings != null) {
          ...
          if (_series.selectionSettings.enable &&
              chart.initialSelectedDataIndexes.isNotEmpty) {
            _series.selectionSettings
                ._selectedDataPointIndex(chart.initialSelectedDataIndexes);
          }
        }
    ...
    _chartWidgets.add(Listener(
        onPointerDown: (PointerDownEvent event) {
          ...
          if (chart._chartSeries.visibleSeries != null &&
            ...
            if (selectionSeries.selectionSettings._selectionRenderer != null) {
        ...
        onPointerMove: (PointerMoveEvent event) {
          ...
          // if ((chart.zoomPanBehavior._isPinching == null
          //     ? true
          //     : !chart.zoomPanBehavior._isPinching) &&
          //         (chart.zoomPanBehavior._canPerformSelection == null ? true : !chart.zoomPanBehavior._canPerformSelection )) {
          //   final Offset position = renderBox.globalToLocal(event.position);
        ...
        onPointerUp: (PointerUpEvent event) {
          ...
          if (chart.tooltipBehavior.enable &&
            ...
            if (chart.tooltipBehavior.builder != null) {
              ...
              if (chart._chartAxis._axisClipRect.contains(position)) {
                ...
                // if (!isTooltipRegion && series._visible) {
          ...
          if ((chart.trackballBehavior != null &&
                  chart.trackballBehavior.enable &&
                  chart.trackballBehavior.shouldAlwaysShow != null &&
          ...
          if ((chart.crosshairBehavior != null &&
            ...
            chart.crosshairBehavior._isLongPressActivated = false;
          }
        },
  ...
  CustomPainter _getSeriesPainter(
    ...
    final CartesianSeries<dynamic, dynamic> series = chart.series[value];
