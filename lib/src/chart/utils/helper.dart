...
Rect _rotatedTextSize(Size size, int angle) {
  ...
  // final Offset shift = rect.topLeft - rotateRect.topLeft;
  // rotateRect = rotateRect.shift(shift);
  return rotateRect;
}
...
num _logBase(num value, num base) {
      ...
      for (int yAxisIndex = 0;
        ...
        for (int ySeriesIndex = 0;
          ...
          if (xAxisSeries == yAxisSeries &&
              (xAxisSeries is ColumnSeries ||
                  xAxisSeries is BarSeries ||
                  xAxisSeries is StackedColumnSeries ||
                  xAxisSeries is StackedBarSeries ||
                  xAxisSeries is StackedAreaSeries ||
                  xAxisSeries is StackedLineSeries ||
                  xAxisSeries is RangeColumnSeries) &&
...
void _canRepaintChartSeries(
  ...
  if (series.animationDuration != oldWidgetSeries.animationDuration ||
      ...
      series.dataLabelSettings.position !=
          oldWidgetSeries.dataLabelSettings.position ||
...
dynamic _getInteractiveTooltipLabel(dynamic value, ChartAxis axis) {
  if (axis is CategoryAxis) {
    ...
    value = _getLabelValue(value, axis, axis.interactiveTooltip.roundingPlace);
  }
  return value;
}
...
void _loadMarkerImage(CartesianSeries<dynamic, dynamic> series) async {
  if (series.markerSettings != null &&
      ...
      series.markerSettings.imageUrl != null) {
...
Future<Null> _init(MarkerSettings markerSettings) async {
  final ByteData data = await rootBundle.load(markerSettings.imageUrl);
